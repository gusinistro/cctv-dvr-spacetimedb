...
export default function DesktopApp() {
  ...
  const [developmentReviewResolved, setDevelopmentReviewResolved] = useState(false);
  ...
  const reviewQueue = hasDevelopmentRole && !sync.connected && !developmentReviewResolved
    ? [{ id: -1, cameraId: 1, task: "objects", classification: "person", confidence: "0.94", biometric: false }]
    : sync.queue;
  ...
  async function review(itemId: number, decision: "approved" | "rejected") {
    if (hasDevelopmentRole && !sync.connected && itemId === -1) {
      setDevelopmentReviewResolved(true);
      setSync((current) => ({ ...current, auditRecords: current.auditRecords + 1, pendingReviews: 0, queue: [] }));
      setNotice(`Fixture de revisão local ${decision === "approved" ? "aprovada" : "rejeitada"}; nenhuma mutação foi enviada ao SpacetimeDB.`);
      return;
    }
