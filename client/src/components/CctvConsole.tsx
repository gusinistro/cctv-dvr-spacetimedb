...
export default function CctvConsole() {
  ...
  const canRunDiagnostics = can(role, "diagnostics");
  ...
  return (
    <div className="cctv-shell">
      ...
      <main className="cctv-main">
        ...
        {screen === "cameras" && canManageCameras && <CamerasScreen cameras={scopedCameras} health={snapshot.cameraHealth} canRunDiagnostics={canRunDiagnostics} onEdit={setCameraModal} onManageInstallation={() => setInstallationModal(selectedInstallation ?? "new")} onCreateInstallation={() => setInstallationModal("new")} onMaintenance={(cameraId, maintenanceStatus, maintenanceDueAt) => protectedCommands.reportCameraHealth(cameraId, maintenanceStatus === "completed", maintenanceStatus === "completed" ? "Manutenção preventiva concluída." : maintenanceStatus === "in_progress" ? "Manutenção preventiva em andamento." : "Manutenção preventiva programada.", maintenanceStatus, maintenanceDueAt)} onStatus={protectedCommands.setCameraStatus} />}
...
function CamerasScreen({ cameras, health, canRunDiagnostics, onEdit, onManageInstallation, onCreateInstallation, onMaintenance, onStatus }: { cameras: Camera[]; health: import("@/lib/cctv/types").CameraHealth[]; canRunDiagnostics: boolean; onEdit: (camera: Camera | "new") => void; onManageInstallation: () => void; onCreateInstallation: () => void; onMaintenance: (cameraId: number, status: import("@/lib/cctv/types").MaintenanceStatus, maintenanceDueAt?: number) => Promise<void>; onStatus: (id: number, status: Camera["status"]) => Promise<void> }) {
  ...
  return <section className="panel-surface content-surface"><div className="section-title spread"><div><h2>Inventário de câmeras</h2><span>Gerencie as fontes, etiquetas, saúde e manutenção preventiva.</span></div><div><button type="button" className="soft-button" onClick={onManageInstallation}>Editar instalação</button><button type="button" className="soft-button" onClick={onCreateInstallation}>Nova instalação</button><button type="button" className="primary-button" onClick={() => onEdit("new")}><Plus size={16} />Adicionar câmera</button></div></div><div className="camera-zone-filter"><span>Grupo / zona</span><div><button type="button" className={zone === "all" ? "active" : ""} onClick={() => setZone("all")}>Todas</button>{zones.map((item) => <button type="button" className={zone === item ? "active" : ""} key={item} onClick={() => setZone(item)}>{item}</button>)}</div></div><div className="camera-list">{listedCameras.map((camera) => { const condition = health.find((item) => item.cameraId === camera.id); const due = maintenanceDue[camera.id] ?? (condition?.maintenanceDueAt ? new Date(condition.maintenanceDueAt).toISOString().slice(0, 10) : ""); const dueAt = due ? new Date(`${due}T12:00:00`).getTime() : undefined; return <article key={camera.id}><div className={`camera-glyph ${camera.status}`}><CameraIcon size={18} /></div><div className="camera-list-main"><strong>{camera.name}</strong><span>{camera.location} · {camera.zone} · {camera.protocol}</span><span>Etiquetas: {camera.tags || "sem etiquetas"} · {condition?.consecutiveFailures ? `${condition.consecutiveFailures} falha(s) consecutiva(s)` : "saudável"}{condition?.maintenanceNote ? ` · ${condition.maintenanceNote}` : ""} · manutenção: {condition?.maintenanceStatus ?? "none"}{condition?.maintenanceDueAt ? ` · vence ${new Date(condition.maintenanceDueAt).toLocaleDateString("pt-BR")}` : ""}</span></div><label className="maintenance-due"><span>Vencimento</span><input aria-label={`Vencimento da manutenção de ${camera.name}`} type="date" value={due} disabled={!canRunDiagnostics} onChange={(event) => setMaintenanceDue({ ...maintenanceDue, [camera.id]: event.target.value })} /></label><span className={`camera-status ${camera.status}`}>{camera.status}</span><button type="button" className="soft-button" onClick={() => void onStatus(camera.id, camera.status === "online" ? "offline" : "online")}>{camera.status === "online" ? "Desativar" : "Reativar"}</button><button type="button" className="soft-button" disabled={!canRunDiagnostics} onClick={() => void onMaintenance(camera.id, condition?.maintenanceStatus === "scheduled" ? "in_progress" : condition?.maintenanceStatus === "in_progress" ? "completed" : "scheduled", dueAt)}>{condition?.maintenanceStatus === "scheduled" ? "Iniciar manutenção" : condition?.maintenanceStatus === "in_progress" ? "Concluir manutenção" : "Agendar manutenção"}</button><button type="button" className="icon-button" onClick={() => onEdit(camera)}><Settings2 size={16} /></button></article>; })}</div></section>;
}
