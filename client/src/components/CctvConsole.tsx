  ...
  return (
    <div className="cctv-shell">
      ...
      <main className="cctv-main">
        ...
        {screen === "cameras" && canManageCameras && <CamerasScreen cameras={scopedCameras} health={snapshot.cameraHealth} canRunDiagnostics={canRunDiagnostics} onEdit={setCameraModal} onManageInstallation={() => setInstallationModal(selectedInstallation ?? "new")} onCreateInstallation={() => setInstallationModal("new")} onMaintenance={(cameraId, maintenanceStatus) => protectedCommands.reportCameraHealth(cameraId, maintenanceStatus === "completed", maintenanceStatus === "completed" ? "Manutenção preventiva concluída." : maintenanceStatus === "in_progress" ? "Manutenção preventiva em andamento." : "Manutenção preventiva programada.", maintenanceStatus)} onStatus={protectedCommands.setCameraStatus} />}
...
function CamerasScreen({ cameras, health, canRunDiagnostics, onEdit, onManageInstallation, onCreateInstallation, onMaintenance, onStatus }: { cameras: Camera[]; health: import("@/lib/cctv/types").CameraHealth[]; canRunDiagnostics: boolean; onEdit: (camera: Camera | "new") => void; onManageInstallation: () => void; onCreateInstallation: () => void; onMaintenance: (cameraId: number, status: import("@/lib/cctv/types").MaintenanceStatus) => Promise<void>; onStatus: (id: number, status: Camera["status"]) => Promise<void> }) {
