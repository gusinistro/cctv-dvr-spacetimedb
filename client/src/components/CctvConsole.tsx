  ...
  return (
    <div className="cctv-shell">
      ...
      <main className="cctv-main">
        ...
        {screen === "cameras" && canManageCameras && <CamerasScreen cameras={scopedCameras} health={snapshot.cameraHealth} onEdit={setCameraModal} onManageInstallation={() => setInstallationModal(selectedInstallation ?? "new")} onCreateInstallation={() => setInstallationModal("new")} onStatus={protectedCommands.setCameraStatus} />}
...
function CamerasScreen({ cameras, health, onEdit, onManageInstallation, onCreateInstallation, onStatus }: { cameras: Camera[]; health: import("@/lib/cctv/types").CameraHealth[]; onEdit: (camera: Camera | "new") => void; onManageInstallation: () => void; onCreateInstallation: () => void; onStatus: (id: number, status: Camera["status"]) => Promise<void> }) {
